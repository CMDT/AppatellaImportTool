-- Description lists the settings table
...
get_settings as (
	select t1.* from settings t1 inner join get_setting_definition_to_setting_mapping t2 on t1.id = t2.setting
)
select * from get_settings;
